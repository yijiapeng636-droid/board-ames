...
export function parseGomokuDecision(content: string): GomokuStrategyDecision {
  ...
  if (!isRecord(value) || typeof value.status !== 'string')
    throw new AgentRuntimeError('invalid_final_status')
  ...
  if (
    !isRecord(value.move) ||
    !Number.isInteger(value.move.row) ||
    !Number.isInteger(value.move.col)
  )
  ...
  const strategy =
    ...
    strategies.includes(value.strategy as (typeof strategies)[number])
      ? (value.strategy as (typeof strategies)[number])
      : 'positional'
  const reason = typeof value.reason === 'string' ? value.reason : ''
  const evidence =
    Array.isArray(value.evidence) && value.evidence.every((item) => typeof item === 'string')
      ? [...new Set(value.evidence.map(chineseEvidence))]
      : []
  return {
    row,
    col,
    strategy,
    reason: reason && !hasChinese(reason) ? 'AI 根据本地局面分析选择了这个落点。' : reason,
    evidence,
  }
}
function validateDecision(decision: GomokuStrategyDecision, context: GomokuAgentContext) {
  if (context.board[decision.row]?.[decision.col] !== 0)
    throw new AgentRuntimeError('invalid_final_move')
  if (
    !context.allowedCandidates.some(
      (candidate) => candidate.row === decision.row && candidate.col === decision.col,
    )
  )
    throw new AgentRuntimeError('move_outside_candidate_set')
}
export function createGomokuFallback(
...
function buildPositionMessage(context: GomokuAgentContext) {
  return JSON.stringify({
    ...
    aiPlayer: context.aiPlayer === 1 ? 'black' : 'white',
    sideToMove: context.sideToMove === 1 ? 'black' : 'white',
    board: context.board.map((line) =>
      line.map((piece) => (piece === 1 ? 'X' : piece === 2 ? 'O' : '.')).join(''),
    ),
    positionInspection: context.positionInspection,
    allowedCandidates: context.allowedCandidates,
    baselineSearch: {
      candidates: context.baselineSearch.candidates,
      forcedMoveType: context.baselineSearch.forcedMoveType,
      metrics: context.baselineSearch.metrics,
    },
    sessionExperience: context.sessionExperience ?? null,
    historicalAnomalies: context.historicalAnomalies ?? null,
  })
}
export async function runGomokuStrategyAgent(
  context: GomokuAgentContext,
  signal?: AbortSignal,
  isContextCurrent?: () => boolean,
  transport: AgentTransport = gomokuAgentTransport,
): Promise<GomokuAgentDecisionResult> {
  const result = await runAgent({
    context,
    messages: [
      {
        role: 'system',
        content: `你是五子棋决策 Agent。思考模式已关闭，本地事实、搜索工具和战术门禁具有最高权威。所有面向玩家的 reason 和 evidence 内容必须使用简体中文；JSON 字段名及 strategy 枚举保持协议规定的英文。\n\n${runtimeSkill}\n\n${gomokuSkill}`,
      },
      {
        role: 'user',
        content: `从允许候选中选择一个落点。只在存在尚未解决的问题时调用工具。最终只返回紧凑的 Decision 或 Fallback JSON，并确保 reason、evidence 使用简体中文。不要复述棋盘、候选列表、PV 或分析过程；reason 不超过 60 个汉字，evidence 最多 3 项。\n${buildPositionMessage(context)}`,
      },
    ],
    tools: gomokuStrategyTools,
    transport,
    budget: GOMOKU_AGENT_BUDGET,
    signal,
    isContextCurrent,
    parseFinal: parseGomokuDecision,
    validateFinal: validateDecision,
    fallback: (reason, currentContext) => createGomokuFallback(currentContext, reason),
  })
  if (import.meta.env.DEV && result.source === 'fallback')
    console.warn('[GomokuAgent fallback]', result.trace)
