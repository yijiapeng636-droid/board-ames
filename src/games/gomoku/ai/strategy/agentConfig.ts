...
import { createPositionKey } from '@/games/gomoku/ai/sessionExperience'
import { searchForcedWinFromMoveInWorker, searchForcedWinInWorker } from '@/games/gomoku/ai/strategy/strategySearchClient'
...
import type { Board, Move, Player, PositionExperienceSummary, SearchResult } from '@/games/gomoku/types/gomoku'

export function buildGomokuAgentContext(board: Board, moves: Move[], aiPlayer: Player, humanPlayer: Player, sideToMove: Player, baselineSearch: SearchResult, sessionExperience?: PositionExperienceSummary): GomokuAgentContext {
  ...
  return {
    ...
    runFixedSearch: (position, move, player, options, signal) => searchFixedCandidateInWorker(position, move, player, options, signal),
    runThreatSearch: searchForcedWinInWorker,
    runThreatSearchFromMove: searchForcedWinFromMoveInWorker,
  }
}

export const GOMOKU_AGENT_BUDGET = { maxModelCalls: 4, maxToolCalls: 4, totalTimeoutMs: 18_000 } as const
