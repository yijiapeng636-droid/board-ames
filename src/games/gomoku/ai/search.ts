...
import { BOARD_SIZE, type AICandidate, type Board, type FixedCandidateSearchResult, type ForcedMoveType, type GomokuDecisionTrace, type Player, type PrincipalVariationMove, type SearchMetrics, type SearchResult, type SearchedCandidate } from '@/games/gomoku/types/gomoku'
...
interface NodeResult { score: number; line: PrincipalVariationMove[]; completed: boolean }
...
function cloneBoard(board: Board): Board { return board.map((line) => [...line]) }
...
function boardKey(board: Board, player: Player) { return `${board.map((line) => line.join('')).join('')}|${player}` }

export function getSearchTerminalScore(board: Board, row: number, col: number, ply: number, rootPlayer: Player = 2): number | null {
...
function orderedCandidates(board: Board, player: Player, limit: number, preferred?: { row: number; col: number }) {
  return generateCandidatePool(board, player, limit).sort((left, right) =>
    ...
    Number(right.row === preferred?.row && right.col === preferred?.col) - Number(left.row === preferred?.row && left.col === preferred?.col) ||
    right.orderingScore - left.orderingScore,
  )
}

function minimax(board: Board, depth: number, currentPlayer: Player, alphaValue: number, betaValue: number, lastMove: { row: number; col: number }, ply: number, extensionLeft: number, context: SearchContext): NodeResult {
  context.metrics.searchedNodes += 1
  const terminal = getSearchTerminalScore(board, lastMove.row, lastMove.col, ply, context.rootPlayer)
  if (terminal !== null) return { score: terminal, line: [], completed: true }
  if (Date.now() >= context.deadline) {
    context.metrics.timedOut = true
    return { score: evaluatePosition(board, context.rootPlayer, currentPlayer), line: [], completed: false }
  }
  ...
  if (cached && cached.depth >= depth) {
    ...
    if (cached.bound === 'exact') return { score: cached.score, line: [...cached.principalVariation], completed: true }
    const applied = applyTTBound(cached, alpha, beta)
    alpha = applied.alpha
    beta = applied.beta
    if (applied.usable) return { score: applied.score, line: [...cached.principalVariation], completed: true }
  }
  ...
  if (depth === 0 && extensionLeft > 0) {
    ...
    const tactical = leafCandidates.some((move) => move.immediateWin || move.blocksImmediateWin || move.forcesReply)
    if (tactical) {
  ...
  for (const candidate of candidates) {
    ...
    const child = minimax(board, effectiveDepth - 1, other(currentPlayer), alpha, beta, candidate, ply + 1, nextExtension, context)
    ...
    if (better) {
      ...
      bestLine = [{ player: playerName(currentPlayer), row: candidate.row, col: candidate.col }, ...child.line]
    }
...
function asSearched(candidate: AICandidate, rootPlayer: Player, searchScore = candidate.orderingScore, line?: PrincipalVariationMove[]): SearchedCandidate {
  return { row: candidate.row, col: candidate.col, staticScore: candidate.orderingScore, searchScore, features: [...candidate.features], principalVariation: line ?? [{ player: playerName(rootPlayer), row: candidate.row, col: candidate.col }] }
}

function metricsFor(candidateCount: number): SearchMetrics {
  return { candidateCount, searchedNodes: 0, searchDepth: 0, searchDurationMs: 0, cutoffCount: 0, cacheHits: 0, ttStores: 0, extensionNodes: 0, timedOut: false }
}

function traceFor(rootPlayer: Player, pool: AICandidate[], metrics: SearchMetrics, forcedMoveType: ForcedMoveType, candidates: SearchedCandidate[], finalSource: GomokuDecisionTrace['finalSource']): GomokuDecisionTrace {
  return {
    aiPlayer: rootPlayer,
    sideToMove: rootPlayer,
    generatedCandidateCount: pool.length,
    candidates: pool.map((item) => ({ row: item.row, col: item.col, attackScore: item.attackScore, defenseScore: item.defenseScore, positionalScore: item.positionalScore, orderingScore: item.orderingScore, features: [...item.features], includedInSearch: candidates.some((candidate) => candidate.row === item.row && candidate.col === item.col) })),
    forcedMoveType,
    search: { completedDepth: metrics.searchDepth, searchedNodes: metrics.searchedNodes, cutoffCount: metrics.cutoffCount, cacheHits: metrics.cacheHits, durationMs: metrics.searchDurationMs, timedOut: metrics.timedOut },
    ...(candidates[0] ? { baselineBest: { row: candidates[0].row, col: candidates[0].col, searchScore: candidates[0].searchScore } } : {}),
    principalVariation: candidates[0]?.principalVariation ?? [],
    finalSource,
  }
}

function finishForced(candidate: AICandidate, type: NonNullable<ForcedMoveType>, rootPlayer: Player, pool: AICandidate[], metrics: SearchMetrics, started: number): SearchResult {
  metrics.searchDurationMs = Date.now() - started
  const candidates = [asSearched(candidate, rootPlayer, type === 'forcedWin' ? SEARCH_WIN_SCORE : candidate.orderingScore)]
...
export function searchPosition(boardInput: Board, options: SearchOptions = {}): SearchResult {
  ...
  const threat = searchForcedWin(board, rootPlayer, { maxPly: options.threatMaxPly ?? SEARCH_CONFIG.threatMaxPly, maxMs: Math.min(SEARCH_CONFIG.threatMaxMs, options.maxMs ?? SEARCH_CONFIG.maxMs) })
  const threatCandidate = threat.forcedWin && threat.winningMove ? pool.find((item) => item.row === threat.winningMove!.row && item.col === threat.winningMove!.col) : undefined
  if (threatCandidate) {
    metrics.searchedNodes = threat.searchedNodes
    metrics.searchDurationMs = Date.now() - started
    metrics.timedOut = threat.timedOut
    const candidates = [asSearched(threatCandidate, rootPlayer, SEARCH_WIN_SCORE - (threat.plyToWin ?? 1), threat.principalVariation)]
  ...
  for (let depth = 1; depth <= maxDepth; depth += 1) {
    ...
    for (const candidate of roots) {
      ...
      const child = minimax(board, depth - 1, other(rootPlayer), -Infinity, Infinity, candidate, 1, context.extensionDepth, context)
      board[candidate.row]![candidate.col] = 0
  ...
  metrics.searchDurationMs = Date.now() - started
  const candidates = completed.slice(0, options.finalCandidateLimit ?? SEARCH_CONFIG.finalCandidateLimit)
  return { candidates, forcedMoveType: null, metrics, trace: traceFor(rootPlayer, pool, metrics, null, candidates, 'search') }
}
...
export function searchFixedCandidate(
  ...
  if (!Number.isInteger(move.row) || !Number.isInteger(move.col) || move.row < 0 || move.row >= BOARD_SIZE || move.col < 0 || move.col >= BOARD_SIZE) throw new Error('固定候选坐标无效')
  ...
  const rootStep: PrincipalVariationMove = { player: playerName(rootPlayer), row: move.row, col: move.col }
  ...
  if (terminal !== null) {
    return {
      ...
      metrics: { searchedNodes: 0, cacheHits: 0, cutoffs: 0, durationMs: Date.now() - started, ttStores: 0, extensionNodes: 0 },
    }
  }
  ...
  for (let depth = 1; depth <= maxDepth; depth += 1) {
    if (Date.now() >= context.deadline) { metrics.timedOut = true; break }
    const child = minimax(board, depth - 1, other(rootPlayer), -Infinity, Infinity, move, 1, context.extensionDepth, context)
